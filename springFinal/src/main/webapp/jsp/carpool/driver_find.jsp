...
<script src="./resources/js/carpool_driver.js"></script>
<style>
   #cplist_wrap {
      margin: auto;
      margin-top: 13%;
...
<%@ include file = "../include/menubar_nologin.jsp" %>
	...
	<div id="cplist_wrap">
		<h2 class="cplist_title">Carpool</h2>
			...
			<form method="get" action="cp_find"
				...
				<table>
					<tr>
						...
						<td><input name="find_name" id="find_name" class="find_text"> 
						<input type="submit" value="검색" class="find_bt"></td>
					</tr>
				</table>
			</form>
		</div>
		...
		<table id="cplist_t">
			<tr>
            	<td colspan="2">
                	<div id="cplist_c"><span>검색된 카풀 수 : ${listcount}</span></div>
            	</td>
         	</tr>
			<tr>
				<td>
		...
		<div class="paging">
			...
			<c:if test="${page > 1 }">
				<a class="direction" href="cp_find?find_name=${find_name}&find_field=${find_field }&page=${page-1}&viewlist=${type}">[이전]</a>&nbsp;
         </c:if>
			...
			<c:forEach var="a" begin="${startpage}" end="${endpage}">
               ...
               <span class="number on">[${a}]</span>
				...
				<c:if test="${a != page }">
					<a class="number" href="cp_find?find_name=${find_name}&find_field=${find_field }&page=${a}&viewlist=${type}">[${a}]</a>&nbsp;
            </c:if>
			</c:forEach>
			...
			<c:if test="${page < maxpage }">
				<a class="direction" href="cp_find?find_name=${find_name}&find_field=${find_field }&page=${page+1}&viewlist=${type}">[다음]</a>
			</c:if>
		</div>
	</div>
</body>
